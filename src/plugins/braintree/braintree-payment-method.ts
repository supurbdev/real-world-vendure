...
export const braintreePaymentMethodHandler = new PaymentMethodHandler({
  ...
  settlePayment: async (ctx, order, payment, args) => {
    const gateway = getGateway(args);
    try {
      const response = await gateway.transaction.submitForSettlement(payment.transactionId);
      return {
  ...
  async createRefund(ctx, input, amount, order, payment, args) {
    const gateway = getGateway(args);
    const response = await gateway.transaction.refund(payment.transactionId, (amount / 100).toString(10));
    if (!response.success) {
      return {
